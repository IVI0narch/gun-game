from random import randrange as rnd, choice
import tkinter as tk
from tkinter import mainloop
import math
import time

# Создание экрана
root = tk.Tk()
fr = tk.Frame(root)
root.geometry('800x600')
canv = tk.Canvas(root, bg='white')
canv.pack(fill=tk.BOTH, expand=1)


class Ball:
    """Класс для создания и обработки состояния снарядов"""
    def __init__(self, x=40, y=450, t=300):
        """ Конструктор класса ball
        Args:
        x - начальное положение мяча по горизонтали
        y - начальное положение мяча по вертикали
        """
        self.time = t
        self.x = x
        self.y = y
        self.r = 10
        self.vx = 0
        self.vy = 0
        self.color = choice(['blue', 'green', 'red', 'brown'])
        self.id = canv.create_oval(self.x - self.r, self.y - self.r,
                                   self.x + self.r, self.y + self.r,
                                   fill=self.color)
        self.live = 120

    def set_coords(self):
        """
        Метод устанавливает координаты шара
        :return:
        """
        canv.coords(self.id, self.x - self.r, self.y - self.r,
                    self.x + self.r, self.y + self.r)

    def move(self):
        """
        Переместить мяч по прошествии единицы времени.
        Метод описывает перемещение мяча за один кадр перерисовки.
        То есть, обновляет значения self.x и self.y с учетом скоростей
        self.vx и self.vy, силы гравитации, действующей на мяч,
        и стен по краям окна (размер окна 800х600).
        """
        self.live -= 1
        if self.live >= 0:
            self.x += self.vx
            self.y -= self.vy
            g = 2  # Ускорение по Oy
            self.vy -= g
            if (self.y + self.vy >= 600) or (self.y + self.vy <= 0):
                self.vy = -self.vy - 5*g
                if self.y >= 600:
                    self.y = 590
                else:
                    self.y = 10
            elif (self.x + self.vx >= 800) or (self.x + self.vx <= 0):
                self.vx = -self.vx
            canv.move(self.id, self.vx, -self.vy)
        else:
            canv.delete(self.id)

    def hittest(self, obj):
        """Функция проверяет сталкивалкивается ли данный мяч с
        целью, описываемой в обьекте obj.
        Args:
            obj: Обьект, с которым проверяется столкновение.
        Returns:
            Возвращает True в случае столкновения мяча и цели.
            В противном случае возвращает False.
        """
        bool_hittest = False
        if ((self.x - obj.x)**2 + (self.y - obj.y)**2) <= \
                ((self.r + obj.r)**2):
            bool_hittest = True
            canv.delete(self.id)
        return bool_hittest


class Gun:
    """Класс для создания и обработки действий пушки"""
    def __init__(self):
        self.f2_power = 10
        self.f2_on = 0
        self.an = 1
        self.id = canv.create_line(20, 450, 50, 420, width=7)

    def fire2_start(self, event):
        """Функция старта подготовки выстрела"""
        self.f2_on = 1

    def fire2_end(self, event):
        """
        Выстрел мячом
        Происходит при отпускании кнопки мыши
        Начальные значения компонент скорости мяча vx и vy зависят
        от положения мыши
        """
        global balls, bullet
        bullet += 1
        new_ball = Ball()
        new_ball.r += 5
        self.an = math.atan((event.y-new_ball.y) / (event.x-new_ball.x))
        new_ball.vx = self.f2_power * math.cos(self.an)
        new_ball.vy = - self.f2_power * math.sin(self.an)
        balls += [new_ball]
        self.f2_on = 0
        self.f2_power = 10

    def targeting(self, event=0):
        """Прицеливание. Зависит от положения мыши"""
        if event:
            self.an = math.atan((event.y-450) / (event.x-20))
        if self.f2_on:
            canv.itemconfig(self.id, fill='orange')
        else:
            canv.itemconfig(self.id, fill='black')
        canv.coords(self.id, 20, 450,
                    20 + max(self.f2_power, 20) * math.cos(self.an),
                    450 + max(self.f2_power, 20) * math.sin(self.an))

    def power_up(self):
        """
        Метод определяет увиличение силы выстрела в зависимости от
        длительности нажатия кнопки мыши
        :return:
        """
        if self.f2_on:
            if self.f2_power < 100:
                self.f2_power += 1
            canv.itemconfig(self.id, fill='orange')
        else:
            canv.itemconfig(self.id, fill='black')


class Score:
    """Класс для подсчета очков"""
    def __init__(self):
        self.points = 0
        self.score = canv.create_text(30, 30, text=self.points, font='32')


class Target:
    """Класс целей"""
    def __init__(self):
        self.points = 0
        self.id = canv.create_oval(0, 0, 0, 0)
        self.x = rnd(600, 780)
        self.y = rnd(300, 550)
        self.r = rnd(2, 50)
        self.vx = rnd(-10, 10)
        self.vy = rnd(-10, 10)
        self.color = 'red'
        canv.coords(self.id, self.x - self.r, self.y - self.r, self.x + self.r,
                    self.y + self.r)
        canv.itemconfig(self.id, fill=self.color)

    def new_target(self):
        """Создание новой цели"""
        canv.coords(self.id, self.x - self.r, self.y - self.r, self.x + self.r,
                    self.y + self.r)
        canv.itemconfig(self.id, fill=self.color)

    def hit(self, score, points=1):
        """Попадание шарика в цель"""
        canv.coords(self.id, -10, -10, -10, -10)
        canv.delete(self.id)
        score.points += points
        canv.itemconfig(score.score, text=score.points)

    def move(self):
        """
        Функция движения целей
        :return:
        """
        self.x += self.vx
        self.y -= self.vy
        if (self.y + self.vy >= 600) or (self.y + self.vy <= 0):
            self.vy = -self.vy
            if self.y >= 600:
                self.y = 590
            else:
                self.y = 10
        elif (self.x + self.vx >= 800) or (self.x + self.vx <= 0):
            self.vx = -self.vx
        canv.move(self.id, self.vx, -self.vy)


screen1 = canv.create_text(400, 300, text='', font='28')
g1 = Gun()
bullet = 0
balls = []


def new_game(event=''):
    """
    Функция начинает новый раунд
    :return:
    """
    global screen1, bullet
    canv.itemconfig(screen1, text='')
    targets = []
    bullet = 0
    for i in range(2):
        targets += [Target()]
    score = Score()
    canv.bind('<Button-1>', g1.fire2_start)
    canv.bind('<ButtonRelease-1>', g1.fire2_end)
    canv.bind('<Motion>', g1.targeting)
    z = 0.03
    while targets or balls:
        for t in targets:
            t.move()
        for i, b in enumerate(balls):
            b.move()
            for j, t in enumerate(targets):
                if b.hittest(t) and balls:
                    balls.pop(i)
                    t.hit(score)
                    targets.pop(j)
            if targets:
                pass
            else:
                for b1 in balls:
                    canv.delete(b1.id)
                canv.update()
                canv.itemconfig(screen1, text='Вы уничтожили цель за ' +
                                              str(bullet) + ' выстрелов')
                canv.itemconfig(score.score, text='')
                canv.update()
                time.sleep(3)
                new_game()
        canv.update()
        time.sleep(z)
        g1.targeting()
        g1.power_up()
    root.after(750, new_game)


new_game()
mainloop()
